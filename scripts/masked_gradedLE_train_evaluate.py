# -*- coding: utf-8 -*-
"""
Script for graded lexical entailmente for masked training
to reproduce the results in paper:
     "No clues, good clues: Out of context Lexical Relation Classification" 
	 
Script usage example:

$python scripts/masked_gradedLE_train_evaluate.py \
...
'hyperlex':{'roberta':{'ant':' contrary', 'cohyp':' coordinated', 
					   #'hyp-4':' subclass', 'hyp-1':' subclass', 'hyp-2':' subclass', 'hyp-3':' subclass', 
					   'hyp':' subclass',
					   'mero':' part', 'no-rel':' random',
					   #'r-hyp-4':' major', 'r-hyp-1':' major', 'r-hyp-2':' major', 'r-hyp-3':' major', 
					   'r-hyp':' major',
					   'syn':' equivalent'},		 
			'bert':{'ant':'contrary', 'cohyp':'coordinated', 
					#'hyp-4':'minor', 'hyp-1':'minor', 'hyp-2':'minor', 'hyp-3':'minor', 
					'hyp':'minor',
					'mero':'part', 'no-rel':'random',
					#'r-hyp-4':'major', 'r-hyp-1':'major', 'r-hyp-2':'major', 'r-hyp-3':'major', 
					'r-hyp':'major',
...
def masked2classification(predictions, label_ids, relation_token_ids, topk=5):
	...
	probs = probs.numpy()
	return (pred_token_ids, real_token_ids, probs, topk_tokens, logits_toks_labels)
...
all_data = load_dataset('csv', data_files=data_files, sep='\t', header=None, names=['source', 'target', 'pos', 'rel', 'grade', 'grade_10'], keep_default_na=False)
...
for train_template, test_template in zip(train_templates, test_templates):
	for i in range(total_repetitions):
		...
		# for val dataset: calculate logits and probs
		logits_val_token_list = []
		probs_val_list = []
		...
		encoded_verb_val.set_format('numpy')
		
		for j in range(math.ceil(len(encoded_verb_val)/batch_pred_size)):
			data_slice = Dataset.from_dict(encoded_verb_val[batch(batch_pred_size,j)])
			p = trainer.predict(data_slice)
			slice_pred_token_ids, slice_real_token_ids, slice_probs, slice_topk_tokens, slice_logits_tokens = masked2classification(p.predictions, p.label_ids, relation_token_ids, topk=TOPK)
			logits_val_token_list.append(slice_logits_tokens)
			probs_val_list.append(slice_probs)
		...
		for j in range(math.ceil(len(encoded_verb_test)/batch_pred_size)):
			...
			logits_token_list.append(slice_logits_tokens)			
			
		# concatenate all slices
		logits_val_tokens = np.concatenate(logits_val_token_list)
		...
		logits_tokens = np.concatenate(logits_token_list)
		
		# fit weights with logit
		X_logit_val = logits_val_tokens
		y_val = encoded_verb_val['grade']
		model_logit = LinearRegression().fit(X_logit_val,y_val)
		weights_logit = model_logit.coef_
		
		# fit weights with prob
		X_prob_val = np.delete(probs_val, 5,axis=1)
		model_prob = LinearRegression().fit(X_prob_val,y_val)
		weights_prob = model_prob.coef_
		
		#calculate score and Spearman correlation
		X_logit_test = logits_tokens
		X_prob_test = np.delete(probs, 5, axis=1)
		sp_corr_logit = spearmanr(model_logit.predict(X_logit_test), encoded_verb_test['grade'])
		# sp_corr_logit and sp_corr_logit_2 should be equal
		sp_corr_logit_2 = spearmanr(np.dot(X_logit_test,weights_logit), encoded_verb_test['grade'])
		sp_corr_prob = spearmanr(model_prob.predict(X_prob_test), encoded_verb_test['grade'])
		...
		print("------")
		print(encoded_verb_test.features['rel_label'])
		print("------")
		...
		print(results_acc)
		encoded_verb_test.set_format('numpy')
		dict_spearman={n:spearmanr(logits_tokens[:,i], encoded_verb_test['grade'], axis=1) for i,n in enumerate(encoded_verb_test.features['rel_label'].names) }
		print(dict_spearman)
		...
		results_words = pd.DataFrame({'pred_label':pred_rel_test_label, 'pred_rel':pred_rel_test, 'pred_verb_token': pred_rel_token_test,
									  ...
									  'source':encoded_verb_test['source'], 'target':encoded_verb_test['target'], 'grade':encoded_verb_test['grade']})
		...
		#for logits
		columns_logits=[ n+"_logit" for n in encoded_verb_test.features['rel_label'].names]	   
		logits_test_df = pd.DataFrame(logits_tokens, columns=columns_logits)	  
		...
		results_words = pd.concat([results_words, logits_test_df, probs_df, chaos_df, nsynsets,topk_tokens_df], axis = 1)
		
		df_y_val = pd.DataFrame({'grade':y_val})
		names_logits_columns = ['logit_'+ n for n in encoded_verb_test.features['rel_label'].names]
		df_logit_val = pd.DataFrame(logits_val_tokens,columns=names_logits_columns)
		names_probs_columns = ['prob_'+ n for n in encoded_verb_test.features['rel_label'].names]
		df_probs_val = pd.DataFrame(probs_val, columns=names_probs_columns)		
		df_validation = pd.concat([df_y_val,df_logit_val,df_probs_val], axis=1)
		...
		with open((fname + '.txt') , 'w') as f:
			...
			print(dict_spearman, file=f)
			print(sp_corr_logit, file=f)
			print(sp_corr_logit_2, file=f)
			print(sp_corr_prob, file=f)
			print({'coefs':weights_logit.tolist(), 'intercept':model_logit.intercept_}, file=f)
			print({'coefs':weights_prob.tolist(), 'intercept':model_prob.intercept_}, file=f)
			
			results_words.to_csv(fname + '.csv', index=False)
			df_validation.to_csv(fname + '.val', index=False)
