...
templates2filename = {"' <W1> ' <SEP> ' <W2> '": '_T1', 
				  " <W1> <SEP> <W2> ": '_T2',
				  "Today, I finally discovered the relation between <W1> and <W2>." : '_T3',
				  "Today, I finally discovered the relation between <W1> and <W2>: <W1> is the <LABEL> of <W2>." : '_T4',
				  }
models2filename = {'bert-large-uncased-whole-word-masking':'_MBert',
				   'roberta-large':'_MRoberta',
				   'roberta-base': '_Mroberta-base',
				   'bert-base-uncased' : "_Mbert-base"
				  }
...
'yulia':{'roberta':{'1':' metaphor', '0':' random'}, 
		 'bert':{'1':'metaphor', '0':'random'}},
'hyperlex':{'roberta':{'ant':' contrary', 'cohyp':' coordinated', 
					   'hyp':' subclass', 'hyp-1':' subclass', 'hyp-2':' subclass', 'hyp-3':' subclass', 'hyp-4':' subclass',
					   'mero':' part', 'no-rel':' random',
					   'r-hyp':' major', 'r-hyp-1':' major', 'r-hyp-2':' major', 'r-hyp-3':' major', 'r-hyp-4':' major',
					   'syn':' equivalent'},		 
			'bert':{'ant':'contrary', 'cohyp':'coordinated', 
					'hyp':'minor', 'hyp-1':'minor', 'hyp-2':'minor', 'hyp-3':'minor', 'hyp-4':'minor',
					'mero':'part', 'no-rel':'random',
					'r-hyp':'major', 'r-hyp-1':'major', 'r-hyp-2':'major', 'r-hyp-3':'major', 'r-hyp-4':'major',
					'syn':'synonym'}}
}
...
all_data = load_dataset('csv', data_files=data_files, sep='\t', header=None, names=['source', 'target', 'pos', 'labels', 'grade', 'grade_10'], keep_default_na=False)
...
for train_template, test_template in zip(train_templates, test_templates):
	for i in range(total_repetitions):
		...
		#predict val: 
		predicciones_val = trainer.predict(test_dataset=encoded_verb_val)
		encoded_verb_val.set_format('numpy')			   
		
		# fit weights with logits
		X_logit_val = predicciones_val.predictions
		...
		probs_val = sfmax(torch.tensor(predicciones_val.predictions))
		...
		X_logit_test = predicciones.predictions
		...
		dict_spearman={n:spearmanr(predicciones.predictions[:,i], encoded_verb_test['grade'], axis=1) for i,n in enumerate(encoded_verb_test.features['labels'].names) }
		print(dict_spearman)
		results_words = pd.DataFrame({'pred_label':pred, 'pred_rel':pred_rel_test, 'real_label':predicciones.label_ids, 
									  'real_rel':real_rel_test, 'source':encoded_verb_test['source'], 
									  'target':encoded_verb_test['target'], 'grade':encoded_verb_test['grade']})
		...
		columns_logits=[ n+"_logit" for n in encoded_verb_test.features['labels'].names]
		logits_test_df = pd.DataFrame(logits_test, columns=columns_logits)
		...
		results_words = pd.concat([results_words, logits_test_df, probs_df, chaos_df,nsynsets], axis = 1)
		
		names_logits_columns = ['logit_'+ n for n in encoded_verb_test.features['labels'].names]
		df_logits_val = pd.DataFrame(predicciones_val.predictions, columns=names_logits_columns)
		names_probs_columns = ['prob_'+ n for n in encoded_verb_test.features['labels'].names]
		df_probs_val = pd.DataFrame(probs_val.numpy(), columns=names_probs_columns)
		df_y_val = pd.DataFrame({'grade':y_val})
		df_validation = pd.concat([df_y_val,df_logits_val,df_probs_val], axis=1)		
